...
def build_with_gox():
    ...
    try:
        ...
        return_code = run_command([
            ...
            "-arch", "amd64",
            "-ldflags", ldflags,
            "-output", "%s/forks-cli_<no value>_<no value>" % output_dir
        ])
        ...
        if return_code == 0:
            ...
            for file in os.listdir(output_dir):
                if file.startswith("forks-cli_windows") and not file.endswith(".exe"):
...
def build_go_app():
    ...
    try:
        ...
        # 设置输出文件名
        output_name = "forks-cli.exe" if os.name == 'nt' else "aibookmark"
